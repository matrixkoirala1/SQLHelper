...
namespace LogicLayers.SQLHelper
{
    /// <summary>
    ...
    public class DataSourceHelper
    {
        ...
        private static int[] GetOrdinals(ArrayList objProperties, IDataReader dr)
        {
            ...
            int[] arrOrdinals = new int[counter + 1];
            int intProperty;
        ...
        public static ArrayList FillCollection(IDataReader dr, Type objType)
        {
            ...
            while (dr.Read())
            {
                ...
                objFillObject = CreateObject(objType, dr);//CreateObject(objType, dr, objProperties, arrOrdinals);
                                                          // objFillObject = CreateObject(objType, dr, objProperties, arrOrdinals);
                                                          // add to collection
        ...
        public static ArrayList FillCollectionExt(IDataReader dr, Type objType)
        {
        ...
        private static T CreateObject<T>(IDataReader dr)
        {
            ...
            if (dr != null)
            {
                ...
                for (int i = 0; i < counter; i++)
                {
                    try
                    {
                        objPropertyInfo = objType.GetProperty(dr.GetName(i), BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
                        if (objPropertyInfo != null && objPropertyInfo.CanWrite)
                        {
                            objValue = Null.SetNull(objPropertyInfo);
                            if (System.Convert.IsDBNull(dr.GetValue(i)))
                            {
                                // translate Null value
                                objPropertyInfo.SetValue(objObject, objValue, null);
                            }
                            else
                            {
                                ...
                                {
                                    objPropertyType = objPropertyInfo.PropertyType;
                                    if (!objPropertyType.IsEquivalentTo(dr[i].GetType()))
                                    {
                                        if (objPropertyType.FullName == "System.Boolean" && dr[i].GetType().FullName == "System.Int32")
                                        {
                                            bool flag = dr.GetValue(i).ToString() == "1" ? true : false;
                                            objPropertyInfo.SetValue(objObject, flag, null);
                                        }
                                        else if (objPropertyType.FullName == "System.Int32" && dr[i].GetType().FullName == "System.String")
                                        {
                                            string colValue = dr.GetValue(i).ToString();
                                            int paraValue = !string.IsNullOrEmpty(colValue) ? Int32.Parse(colValue) : 0;
                                            objPropertyInfo.SetValue(objObject, paraValue, null);
                                        }
                                        else if (objPropertyType.FullName.Contains("System.Nullable") && dr[i].GetType().FullName == "System.Boolean")
                                        {
                                            bool flag = dr.GetValue(i).ToString() == "1" ? true : false;
                                            objPropertyInfo.SetValue(objObject, flag, null);
                                        }
                                        ...
                                        {
                                            objPropertyInfo.SetValue(objObject, Convert.ChangeType(dr.GetValue(i), objPropertyType), null);
                                        }
                                    }
                                    else
                                    {
                                        objPropertyInfo.SetValue(objObject, dr.GetValue(i), null);
                                    }
                                }
                                catch
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (objPropertyType.BaseType.Equals(typeof(System.Enum)))
                                        {
                                            // check if value is numeric and if not convert to integer ( supports databases like Oracle )
                                            int testint = 0;
                                            if (testint.GetType() == dr.GetValue(i).GetType())
                                            {
                                                objPropertyInfo.SetValue(objObject, System.Enum.ToObject(objPropertyType, Convert.ToInt32(dr.GetValue(i))), null);
                                            }
                                            else
                                            {
                                                objPropertyInfo.SetValue(objObject, System.Enum.ToObject(objPropertyType, dr.GetValue(i)), null);
                                            }
                                        }
                                        else
                                        {
        ...
        private static object CreateObject(Type objType, IDataReader dr)
        {
            ...
            if (dr != null)
            {
                for (int i = 0; i < dr.FieldCount; i++)
                {
                    ...
                    objPropertyInfo = objType.GetProperty(dr.GetName(i), BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance); //GetPropertieExactName(objType, dr.GetName(i));
                                ...
                                catch
                                {
                                    try
                                    {
                                        if (objPropertyType.BaseType.Equals(typeof(System.Enum)))
                                        {
                                            // check if value is numeric and if not convert to integer ( supports databases like Oracle )
                                            int test = 0;
                                            if (test.GetType() == dr.GetValue(i).GetType())
                                            {
                                                objPropertyInfo.SetValue(objObject, System.Enum.ToObject(objPropertyType, Convert.ToInt32(dr.GetValue(i))), null);
                                            }
                                            else
                                        ...
                                        {
                                            // guid is not a datatype common across all databases ( ie. Oracle )                            
                                            objPropertyInfo.SetValue(objObject, Convert.ChangeType(new Guid(dr.GetValue(i).ToString()), objPropertyType), null);
                                        }
                                        else
                                        {
                                            // try explicit conversion
